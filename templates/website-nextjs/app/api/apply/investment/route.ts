...
import { investmentApplicationSchema, type ApplyResponse } from '@/lib/schemas';

/**
 * Pre-qualification intake. Runs server-side only; the client never touches a
 * database.
 *
 * TODO(supabase): insert the parsed payload into the pipeline table with the
...
export async function POST(request: Request): Promise<NextResponse<ApplyResponse>> {
  ...
  const parsed = investmentApplicationSchema.safeParse(body);
  if (!parsed.success) {
